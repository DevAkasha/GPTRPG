...
public abstract class Interactable<T> : Interactable where T : InteractableData
{
    ...
    protected virtual void OccupidPlayer(PlayerSpirit player) { }
    protected virtual void ApplyPlayer(PlayerSpirit player) { }
    protected virtual void OccupidNPC(NPCSpirit npc) { }
    protected virtual void ApplyNPC(NPCSpirit npc) { }
    ...
    private void OnTriggerEnter(Collider collider)
    {
        ...
        {
            case "Player":
                PlayerSpirit player = collider.GetComponent<PlayerSpirit>();
                if (primer)
                {
                    ApplyPlayer(player);
            ...
            case "NPC":
                NPCSpirit NPC = collider.GetComponent<NPCSpirit>();
                if (primer)
                {
                    ApplyNPC(NPC);
                    if (isDisposable) DestroyAfterDelay(applyDestroyDelay);
